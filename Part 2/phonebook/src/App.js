...
const App = () => {
  ...
  const addContact = (event) =>{
    ...
    if(personsArray.includes(personObj.name)){
      ...
      axios.post('http://localhost:3001/persons', personObj).then(response => setPersons(persons.concat(response.data)))
      setNewName(''); 
      setNewNum('');
  ...
  const deleteContact = (n) =>{
    if(window.confirm("Delete "+ n.name + '?')){
      persons.forEach(element => {
        if(element === n){
          axios.delete('http://localhost:3001/persons/' + n.id).then(setPersons(persons.filter(p => p.id !== n.id)))
